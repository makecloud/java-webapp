...
//记住密码
var r_label = $('.adv_pwd_btn');
var r_bg = $('.adv_pwd_btn i');
...
//检查是否有cookie
var advuname = $('#advuname');
var advupwd = $('#advupwd');
var cookieName = 'username';
var cookiePwd = 'userpwd';
if ($.cookie(cookieName) && $.cookie(cookiePwd)) {
  advuname.val($.cookie(cookieName));
  advupwd.val($.cookie(cookiePwd));
}

//验证姓名.密码.验证码
var advucode = $('#advucode');
var adv_login_btn = $('.adv_login_btn a');
var regName = /^[0-9a-zA-Z]{2,12}$/; //由字母和数字组成
var regPwd = /^[0-9]{6,12}$/;
var error = $('.error');
var error_text = $('.error .error_text');
adv_login_btn.click(function (e) {
  if (!(advuname.val()) || !regName.test(advuname.val())) {
    e.preventDefault();
    error_text.html('用户名不正确!');
    error.css({'top': '38px'});
    error.stop().animate({'opacity': '1'}, 300);
    timer();
  } else if (!(advupwd.val()) || !regPwd.test(advupwd.val())) {
    e.preventDefault();
    error_text.html('密码填写不正确!');
    error.css('top', '84px');
    error.stop().animate({'opacity': '1'}, 300);
    timer();
  } else if (advucode.val().toUpperCase() != codespace.html().toUpperCase()) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.code == 0) {
          ...
          if (r_bg.hasClass('r_pwd_yes')) {
            $.cookie('username', advuname.val(), {expires: 30});
            $.cookie('userpwd', advupwd.val(), {expires: 30});
          } else {
